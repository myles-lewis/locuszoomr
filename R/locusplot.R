...
#' LD plot
#' 
#' Linkage disequilibrium (LD) plot at a gene locus.
#' 
#' @details 
#' This function queries LDlink (https://ldlink.nci.nih.gov/) via the [LDlinkR] 
#' package to retrieve LD information at a locus from 1000 Genomes. A maximum 
#' of 1000 SNPs can be queried. The [LD.plot] function from the gaston package 
#' is used to produce an LD plot at the gene locus.
#' 
#' @param locus Object of class 'locus' generated by [locus()].
#' @param LD Logical whether LD is plotted. Queries 1000 genomes via LDlinkR 
#' package. See [LDlinkR]. Results are cached using the `memoise` package, so
...
#' @param r2d Either "r2" for LD r^2 or "d" for LD D', default = "r2". Passed 
#' to [LDlinkR::LDmatrix].
#' @param token Peronsal access token for accessing 1000 genomes LD data via 
#' LDlink API. See [LDlinkR].
#' @param ... Further arguments passed to [gaston::LD.plot].
#' @return No return value.
#' @importFrom LDlinkR LDmatrix
#' @importFrom memoise memoise
...
LDplot <- function(loc,
  ...
  rownames(ldm) <- ldm$RS_number
  ld_pos <- data[match(ldm$RS_number, data[, labs]), loc$pos]
  ldm <- ldm[, -1]
  sum_NA <- apply(ldm, 1, function(i) sum(is.na(i)))
  NA_rows <- sum_NA == nrow(ldm)
  ldm <- ldm[!NA_rows, !NA_rows]
  if (auto & nrow(ldm) > 100) {
    gaston::LD.plot(LD = ldm, snp.positions = ld_pos, 
                    write.snp.id = FALSE, 
                    draw.chr = FALSE,
