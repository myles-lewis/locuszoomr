...
gg_scatter <- function(loc,
                       ...
                       showLD = TRUE,
                       LD_scheme = c(
                           "grey", "royalblue", "cyan2", "green3",
                           "orange", "red", "purple"
                       ),
                       recomb_col = "blue",
                       recomb_linewidth = 0.35,
                       recomb_offset = 0,
                       legend_pos = "topleft",
                       labels = NULL,
                       eqtl_gene = NULL,
    ...
    if (!is.null(legend_pos)) {
        ...
        if (showLD & hasLD) {
            ...
            legend_labels <- rev(c(
                "Index SNP", "0.8 - 1.0", "0.6 - 0.8", "0.4 - 0.6",
                "0.2 - 0.4", "0.0 - 0.2", "NA"
            ))
    ...
    if (!is.null(labels)) {
        ...
        if (any(is.na(text_label_ind))) {
            message(
                "label ", paste(labels[is.na(text_label_ind)], collapse = ", "),
                " not found"
            )
        }
    }
    ...
    if (!recomb) {
        ...
        p <- p +
            ...
            theme_classic() +
            theme(
                axis.text = element_text(colour = "black", size = 10 * cex.axis),
