...
`recomb1000GAvg.bw`. For hg19, the link is
http://hgdownload.soe.ucsc.edu/gbdb/hg19/decode/ and the default track we use is
`hapMapRelease24CombinedRecombMap.bw`. The file can then be loaded into R as a
`GRanges` object using `import.bw()`, and then used directly by `link_recomb()`
as follows.
