...
called to retrieve LD information at the locus which is overlaid on the locus
plot.
...
acquired from LDlink and included in the `r2` column. This can be plotted by
setting `LD = "r2"`. This allows users to add their own LD information from
...
The gene tracks can be customised. See the help page at `?plot.locus` for more
details.
