...
Erythematosus (SLE) by Bentham et al (2015). `LD = FALSE` is set to avoid the
...
loc <- locus(SLE_gwas_sub, gene = 'UBE2L3', flank = 1e5, LDtoken = "your_token")
plot(loc)
```
...
The gene tracks can be customised.
...
plots side by side, specify layout manually and set the argument `use_layout =
FALSE` when plotting locus objects.

```{r, fig.dim = c(10,5)}

loc2 <- locus(SLE_gwas_sub, gene = 'STAT4', flank = 1e5, LD = FALSE)
loc3 <- locus(SLE_gwas_sub, gene = 'IRF5', flank = c(7e4, 2e5), LD = FALSE)
