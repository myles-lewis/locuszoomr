...
These gene annotation plots are produced via R base graphics system. 'ggplot2'
and 'plotly' versions can also be generated.
...
specified using the argument `fix_window`. Finally, the locus can be specified
...
Additional arguments can also be passed onto `plot()` via the `...` system. For
example `col = NA` can be added to `locus_plot()` or `scatter_plot()` to remove
the black outline around the scatter plot markers.
